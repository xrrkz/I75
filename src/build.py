...
/* Shrink-wrap to the headline: stretched to the container, the slab's right
   edge lands well past the last glyph and reads as a stray grey rectangle,
   because Anton is far narrower than the fallback the box was tuned against.
   Sizing the wrapper to the text keeps the slab behind the words at any width. */
.h1wrap{position:relative;align-self:flex-start;max-width:100%}
.h1bg{position:absolute;left:-40px;right:8%;top:6%;bottom:10%;background:#1c1c1c;transform:skewX(-8deg)}
